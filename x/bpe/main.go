// BPE, inspired by  https://eli.thegreenplace.net/2024/tokens-for-llms-byte-pair-encoding-in-go/
package main
...
var (
	vocabSize = flag.Int("S", 500, "vocab size")
)
...
func main() {
	...
	var (
		text  = string(b)
		vocab = Train(text, *vocabSize, CL100KBaseSplitPattern)
	)
	for k, v := range vocab {
		fmt.Printf("%v => %v\n", k, v)
	}
...
func Train(text string, vocabSize int, splitPattern string) map[string]int {
	...
	for len(vocab) < vocabSize {
		...
		log.Printf("%v/%v [%v]", len(vocab), vocabSize, newToken)
